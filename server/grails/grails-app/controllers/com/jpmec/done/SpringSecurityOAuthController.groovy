...
class SpringSecurityOAuthController {
    ...
    def linkAccount = { OAuthLinkAccountCommand command ->
        ...
        if (request.post) {
            boolean linked = command.validate() && SecureUser.withTransaction { status ->
                SecureUser user = SecureUser.findByUsernameAndPassword(
    ...
    def createAccount = { OAuthCreateAccountCommand command ->
        ...
        if (request.post) {
            if (!springSecurityService.loggedIn) {
                ...
                boolean created = command.validate() && SecureUser.withTransaction { status ->
                    SecureUser user = new SecureUser(username: command.username, password: command.password1, enabled: true)
                    user.addToOAuthIDs(provider: oAuthToken.providerName, accessToken: oAuthToken.socialId, user: user)
                    ...
                    for (roleName in config.oauth.registration.roleNames) {
                        SecureUserSecureRole.create user, SecureRole.findByAuthority(roleName)
                    }
    ...
    protected OAuthToken updateOAuthToken(OAuthToken oAuthToken, SecureUser user) {
...
/*
    private def updateUser(SecureUser user, OAuthToken oAuthToken) {
        if (!user.validate()) {
            return
        }
...
class OAuthCreateAccountCommand {
    ...
    static constraints = {
        username blank: false, validator: { String username, command ->
            SecureUser.withNewSession { session ->
                if (username && SecureUser.countByUsername(username)) {
