...
class OpenIdController {
	...
	protected boolean createNewAccount(String username, String password, String openId) {
		boolean created = SecureUser.withTransaction { status ->
			def config = SpringSecurityUtils.securityConfig

			password = encodePassword(password)
			def user = new SecureUser(username: username, password: password, enabled: true)
	...
	protected void registerAccountOpenId(String username, String password, String openId) {
		...
		SecureUser.withTransaction { status ->
			def user = SecureUser.findByUsername(username)
			user.addToOpenIds(url: openId)
			if (!user.validate()) {
